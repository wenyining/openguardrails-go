...
func (ac *AsyncClient) CheckPromptWithModelAsync(ctx context.Context, content, model string) <-chan AsyncResult[*GuardrailResponse] {
	...
	if ac.closed {
		ac.closeMu.RUnlock()
		resultChan <- AsyncResult[*GuardrailResponse]{Error: NewOpenGuardrailsError("async client is closed", nil)}
		close(resultChan)
		return resultChan
	}
	ac.closeMu.RUnlock()
...
// Example:
//
//	conversations := [][]*openguardrails.Message{
//		{openguardrails.NewMessage("user", "Question 1")},
//		{openguardrails.NewMessage("user", "Question 2")},
//	}
...
func (ac *AsyncClient) HealthCheckAsync(ctx context.Context) <-chan AsyncResult[map[string]interface{}] {
	...
	if ac.closed {
		ac.closeMu.RUnlock()
		resultChan <- AsyncResult[map[string]interface{}]{Error: NewOpenGuardrailsError("async client is closed", nil)}
		close(resultChan)
		return resultChan
	}
	ac.closeMu.RUnlock()
