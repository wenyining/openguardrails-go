...
const (
	...
	DefaultBaseURL = "https://api.openguardrails.com/v1"
	// DefaultModel Default model name
	DefaultModel = "OpenGuardrails-Text"
	// DefaultTimeout Default request timeout (seconds)
	DefaultTimeout = 30
	// DefaultMaxRetries Default maximum retry count
	DefaultMaxRetries = 3
	// UserAgent User agent
	UserAgent = "openguardrails-go/3.0.0"
)

// Client OpenGuardrails client - Context-aware AI guardrail based on LLM
//
...
//	messages := []*openguardrails.Message{
//		openguardrails.NewMessage("user", "问题"),
//		openguardrails.NewMessage("assistant", "回答"),
//	}
...
//   - OpenGuardrailsError: Other API errors
//
// Example:
//
...
//	messages := []*openguardrails.Message{
//		openguardrails.NewMessage("user", "User question"),
//		openguardrails.NewMessage("assistant", "Assistant answer"),
//	}
...
func (c *Client) CheckPromptImage(ctx context.Context, prompt, image string, userID ...string) (*GuardrailResponse, error) {
	return c.CheckPromptImageWithModel(ctx, prompt, image, "OpenGuardrails-VL", userID...)
}
...
func (c *Client) CheckPromptImages(ctx context.Context, prompt string, images []string, userID ...string) (*GuardrailResponse, error) {
	return c.CheckPromptImagesWithModel(ctx, prompt, images, "OpenGuardrails-VL", userID...)
}
...
func (c *Client) makeRequestWithData(ctx context.Context, method, endpoint string, requestData interface{}) (*GuardrailResponse, error) {
	...
	for attempt := 0; attempt <= c.maxRetries; attempt++ {
		...
		default:
			...
			lastErr = NewOpenGuardrailsError(fmt.Sprintf("API request failed with status %d: %s", resp.StatusCode(), errorMsg), nil)
			if attempt < c.maxRetries {
				c.sleep(ctx, c.calculateBackoff(attempt))
				continue
			}
...
func (c *Client) handleErrorResponse(resp *resty.Response) error {
	...
	default:
		...
		return NewOpenGuardrailsError(fmt.Sprintf("API request failed with status %d: %s", resp.StatusCode(), errorMsg), nil)
	}
}
